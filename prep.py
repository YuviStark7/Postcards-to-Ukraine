...
import os
# folder holding the original "DASHA LETTER n.png" files. Looked for next to this
# script, then one level up (they live in the Dasha folder, this script in the site
# folder inside it). Override with:  DASHA_SRC=/some/path python3 prep.py
HERE=os.path.dirname(os.path.abspath(__file__))
def _find_src():
    if os.environ.get('DASHA_SRC'): return os.environ['DASHA_SRC']
    for d in (HERE, os.path.dirname(HERE), os.getcwd()):
        if os.path.exists(os.path.join(d,'DASHA LETTER 1.png')): return d
    raise SystemExit('Could not find "DASHA LETTER 1.png". Set DASHA_SRC to the folder holding it.')
SRC=_find_src()
...
# the vodka bottle that rides along the writing edge
vod=next((q for q in (os.path.join(HERE,'Vodka.png'), os.path.join(SRC,'Vodka.png'))
          if os.path.exists(q)), None)
if vod:
    v=np.array(Image.open(vod).convert('RGBA'))
    vy,vx=np.where(v[...,3]>8)
    vc=Image.fromarray(v[vy.min():vy.max()+1, vx.min():vx.max()+1])
    vc=vc.resize((240, round(vc.height*240/vc.width)), Image.LANCZOS)
    vc.save(f'{OUT}/vodka.webp', quality=88, method=6)

D={'w':1067,'h':806,'lines':{str(k):v for k,v in meta.items()}}
json.dump(D,open(f'{OUT}/lines.json','w'))
open(os.path.join(HERE,'data.js'),'w').write('/* generated by prep.py — text-line geometry for the handwriting reveal */\nwindow.__LINES = '+json.dumps(D,separators=(',',':'))+';\n')
print('done')
